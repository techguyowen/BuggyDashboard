...
test('Financial Equivalence Exhaustive Stress Test: All 1,000,000 cent increments from $0.00 to $10,000.00', () => {
  let matchedCount = 0;
  const totalCents = 1000000; // 0 to 10000.00 in 0.01 increments

  for (let cents = 0; cents <= totalCents; cents++) {
    const bidVal = cents / 100;
    const retailVal = bidVal > 0 ? bidVal * 2 : null;
  ...
  assert.strictEqual(matchedCount, 1000001);
});
